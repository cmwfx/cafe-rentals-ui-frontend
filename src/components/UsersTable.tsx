...
import { fetchUsersWithProfiles, UserWithProfile } from "@/functions/admin";
...
const UsersTable = () => {
	...
	const fetchUsers = async () => {
		try {
			...
			// Fetch users with profiles from the Edge Function
			const userProfiles = await fetchUsersWithProfiles();
		...
		<div className="space-y-4">
			<div className="flex items-center pb-4">
				<Input
					...
					className="max-w-sm"
				/>
				<Button variant="outline" onClick={() => fetchUsers()} className="ml-2">
					Refresh
				</Button>
			</div>
										...
										<TableCell className="w-1/4">
											<div className="flex items-center space-x-2">
												<Input
													...
													onChange={(e) =>
														handleCreditAmountChange(user.id, e.target.value)
													}
													type="number"
													min="0"
													step="0.01"
												...
												<Button
													...
													disabled={
														addingCredit[user.id] || !creditAmounts[user.id]
													}
													size="sm"
												>
